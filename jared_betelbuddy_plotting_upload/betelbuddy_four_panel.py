...
def calc_abs_mag_from_flux(flux_density, distance_pc, zp_flux):
...
for montarges_flux in montarges_fluxes:
    ...
    montarges_mag = calc_abs_mag_from_flux(montarges_flux, 168, Cnt_Ha_ZP)
    # print(montarges_mag)
    montarges_mags.append(montarges_mag)


MAMA25_FUV_ZP = 3.92008e-9  # SVO, erg cm^-2 Angstrom^-1 for Vega system
HST_fluxes = [1e-14, 1e-16] # erg cm^-2 Angstrom^-1, estimated upper bound, ish 
HST_mags = []
for HST_flux in HST_fluxes:
    HST_flux = HST_flux  # Converting from W m^-2 um^-1 to erg s^-1 cm^-2 Angstrom^-1
    HST_mag = calc_abs_mag_from_flux(HST_flux, 168, MAMA25_FUV_ZP)
    # print(montarges_mag)
    HST_mags.append(HST_mag)

# --- NOW ONTO SOME PLOTTING ---


# IMPORTANT!!! CHANGE PATH TO WHEREVER YOU'RE STORING DR DENNIS' TRACKS
directories = ["DATA/Bband/history_m*.data",
               "DATA/zimpol/history_m*.data",
               "DATA/alopeke/history_m*.data",
               "DATA/HST_stis_fuv/history_m*.data",]
...
filters = [
    ...
    {'filter':'25MAMA','x_var': 'log_Teff', 'y_var': 'Mag_bol_filter', 'x_title': r'$\log T_{eff}$', 'y_title': r'$M_{\text{abs}}$ HST FUV-STIS', 
         ...
         'shades': [
            {
                'x_lims': None,        # Teff constraints
                'y_lims': HST_mags,            # Magnitude constraints (calculated earlier)
                'label': 'Goldberg+ 2025', 
                'color': 'red', 
                'alpha': 0.4,
                'upperlimit': True
...
# NAME IT HOW YA LIKE
savename = "four_buddies_grid_jared.png"
